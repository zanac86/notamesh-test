...
//  Xn = X-(2*Y); Yn = Y+(X/1.1);
  ...
  if ((EndLed - StartLed)>=10) {
    ...
    leds[X%((EndLed - StartLed))] = ColorFromPalette(gCurrentPalette, thisindex, 255, currentBlending);
    fadeToBlackBy(&leds[StartLed], (EndLed - StartLed), 16);                     // 8 bit, 1 = slow, 255 = fast
  }  
} // serendipitous_pal()
